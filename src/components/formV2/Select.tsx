...
export type SelectProps<
  ...
  V = any,
> = Omit<SelectAntdProps<T, ISelectOptions<T, V>>, "direction" | "onChange"> &
  CommonFormItemProps & {
    ...
    onChange?: (
      value: T,
      option: ISelectOptions<T, V> | ISelectOptions<T, V>[],
      ...event: any[]
    ) => void;
...
export function Select<T extends SelectValueType = SelectValueType, V = any>({
  ...
  const defaultSearchFilter = (
    input: string,
    option: ISelectOptions<T, V> | undefined,
  ) => {
    ...
    if (typeof value !== "string") {
      console.error(
        "please implement custom filterOption for using search function",
      );
      return true;
    }
  ...
  const handleClickOutside = (e: MouseEvent | TouchEvent) => {
    if (buttonWrapperRef.current?.contains(e.target as HTMLElement)) return;
    setIsOpen(false);
  };
